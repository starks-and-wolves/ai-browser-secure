...
class BrowserSession(BaseModel):
	...
	_user_approval_watchdog: Any | None = PrivateAttr(default=None)
	...
	async def reset(self) -> None:
		...
		self._user_approval_watchdog = None
		if self._demo_mode:
	...
	async def on_NavigateToUrlEvent(self, event: NavigateToUrlEvent) -> None:
		...
		# Check user approval BEFORE navigation if watchdog is enabled
		if self._user_approval_watchdog is not None:
			# Extend the event timeout to allow for user approval (60s for UI + buffer)
			if event.event_timeout is not None and event.event_timeout < 90.0:
				event.event_timeout = 90.0
				self.logger.debug(f'Extended event timeout to {event.event_timeout}s for user approval')

			try:
				await self._user_approval_watchdog.check_navigation_approval(event)
			except Exception as e:
				# Handle both PermissionDeniedError and ValueError
				# PermissionDeniedError contains context for re-planning
				self.logger.warning(f'Navigation blocked by user approval: {e}')
				raise
	...
	async def attach_all_watchdogs(self) -> None:
		...
		# Initialize UserApprovalWatchdog FIRST (handles user approval for sensitive actions - IDE-like security prompts)
		# Must be initialized before other watchdogs so its handlers run first and can block actions
		if self.browser_profile.require_user_approval:
			from browser_use.browser.watchdogs.user_approval_watchdog import UserApprovalWatchdog

			UserApprovalWatchdog.model_rebuild()
			self._user_approval_watchdog = UserApprovalWatchdog(
				event_bus=self.event_bus,
				browser_session=self,
				require_approval_for_navigation=self.browser_profile.user_approval_for_navigation,
				require_approval_for_forms=self.browser_profile.user_approval_for_forms,
				require_approval_for_sensitive_data=self.browser_profile.user_approval_for_sensitive_data,
				require_approval_for_file_operations=self.browser_profile.user_approval_for_file_operations,
				approved_domains=self.browser_profile.user_approval_pre_approved_domains,
				denied_domains=self.browser_profile.user_approval_denied_domains,
			)
			self._user_approval_watchdog.attach_to_session()
			self.logger.info('🔐 User approval mode enabled - sensitive actions will require user confirmation')
