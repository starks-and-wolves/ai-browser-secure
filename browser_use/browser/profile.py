...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	require_user_approval: bool = Field(
		default=False,
		description='Enable user approval mode for sensitive actions (navigation, form filling, etc.). Similar to IDE command approval.',
	)
	user_approval_for_navigation: bool = Field(
		default=True,
		description='When require_user_approval=True, require approval for navigation to new URLs.',
	)
	user_approval_for_forms: bool = Field(
		default=False,
		description='When require_user_approval=True, require approval for all form inputs (not just sensitive fields).',
	)
	user_approval_for_sensitive_data: bool = Field(
		default=True,
		description='When require_user_approval=True, require approval when entering sensitive/PII data (passwords, credit cards, etc.).',
	)
	user_approval_for_file_operations: bool = Field(
		default=True,
		description='When require_user_approval=True, require approval for file uploads.',
	)
	user_approval_pre_approved_domains: list[str] = Field(
		default_factory=list,
		description='Domains that are pre-approved for navigation (skip approval prompts). Supports wildcards like "*.google.com".',
	)
	user_approval_denied_domains: list[str] = Field(
		default_factory=list,
		description='Domains that are always denied (block navigation). Supports wildcards like "*.malicious.com".',
	)
