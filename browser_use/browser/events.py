...
class NavigateToUrlEvent(BaseEvent[None]):
	...
	url: str = Field(min_length=1, description='URL to navigate to (must not be empty)')
	...
	# Increased default from 15s to 30s to handle slow-loading sites (e.g., Render free tier cold starts)
...
class BrowserStartEvent(BaseEvent):
	...
	# Browser start can involve installing/downloading a browser (e.g. Playwright) and is often slow on cold starts.
	# Keep this reasonably high by default; it remains fully configurable via TIMEOUT_BrowserStartEvent.
	event_timeout: float | None = Field(default_factory=lambda: _get_timeout('TIMEOUT_BrowserStartEvent', 300.0))  # seconds
...
class BrowserLaunchEvent(BaseEvent[BrowserLaunchResult]):
	...
	# Local browser launch may trigger a Playwright install/download on first run.
	# Keep this high by default; it remains fully configurable via TIMEOUT_BrowserLaunchEvent.
	event_timeout: float | None = Field(default_factory=lambda: _get_timeout('TIMEOUT_BrowserLaunchEvent', 180.0))  # seconds
