...
class NavigateToUrlEvent(BaseEvent[None]):
    ...
    url: str = Field(
        min_length=1, description='URL to navigate to (must not be empty)')
    wait_until: Literal['load', 'domcontentloaded',
                        'networkidle', 'commit'] = 'load'
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_NavigateToUrlEvent', 15.0))  # seconds
...
class ClickElementEvent(ElementSelectedEvent[dict[str, Any] | None]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_ClickElementEvent', 15.0))  # seconds
...
class ClickCoordinateEvent(BaseEvent[dict]):
    ...
    coordinate_y: int
    button: Literal['left', 'right', 'middle'] = 'left'
    # If True, skip safety checks (file input, print, select)
    force: bool = False

    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_ClickCoordinateEvent', 15.0))  # seconds
...
class TypeTextEvent(ElementSelectedEvent[dict | None]):
    ...
    # Name of the sensitive key being typed (e.g., 'username', 'password')
...
class SwitchTabEvent(BaseEvent[TargetID]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_SwitchTabEvent', 10.0))  # seconds
...
class ScreenshotEvent(BaseEvent[str]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_ScreenshotEvent', 15.0))  # seconds
...
class BrowserStateRequestEvent(BaseEvent[BrowserStateSummary]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserStateRequestEvent', 30.0))  # seconds
...
class GoForwardEvent(BaseEvent[None]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_GoForwardEvent', 15.0))  # seconds
...
class UploadFileEvent(ElementSelectedEvent[None]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_UploadFileEvent', 30.0))  # seconds
...
class GetDropdownOptionsEvent(ElementSelectedEvent[dict[str, str]]):
    ...
    event_timeout: float | None = Field(
        default_factory=lambda: _get_timeout(
...
class SelectDropdownOptionEvent(ElementSelectedEvent[dict[str, str]]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_SelectDropdownOptionEvent', 8.0))  # seconds
...
class ScrollToTextEvent(BaseEvent[None]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_ScrollToTextEvent', 15.0))  # seconds
...
class BrowserStartEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserStartEvent', 120.0))  # seconds
...
class BrowserStopEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserStopEvent', 45.0))  # seconds
...
class BrowserLaunchEvent(BaseEvent[BrowserLaunchResult]):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserLaunchEvent', 30.0))  # seconds
...
class BrowserKillEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserKillEvent', 30.0))  # seconds
...
class BrowserConnectedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserConnectedEvent', 30.0))  # seconds
...
class BrowserStoppedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserStoppedEvent', 30.0))  # seconds
...
class TabCreatedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_TabCreatedEvent', 30.0))  # seconds
...
class TabClosedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_TabClosedEvent', 10.0))  # seconds
...
class AgentFocusChangedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_AgentFocusChangedEvent', 10.0))  # seconds
...
class TargetCrashedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_TargetCrashedEvent', 10.0))  # seconds
...
class NavigationStartedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_NavigationStartedEvent', 30.0))  # seconds
...
class NavigationCompleteEvent(BaseEvent):
    ...
    # Detailed loading status (e.g., network timeout info)
    loading_status: str | None = None

    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_NavigationCompleteEvent', 30.0))  # seconds
...
class BrowserErrorEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_BrowserErrorEvent', 30.0))  # seconds
...
class SaveStorageStateEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_SaveStorageStateEvent', 45.0))  # seconds
...
class StorageStateSavedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_StorageStateSavedEvent', 30.0))  # seconds
...
class LoadStorageStateEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_LoadStorageStateEvent', 45.0))  # seconds
...
class StorageStateLoadedEvent(BaseEvent):
    ...
    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_StorageStateLoadedEvent', 30.0))  # seconds
...
class FileDownloadedEvent(BaseEvent):
    ...
    # Whether this was an automatic download (e.g., PDF auto-download)
    auto_download: bool = False

    event_timeout: float | None = Field(default_factory=lambda: _get_timeout(
        'TIMEOUT_FileDownloadedEvent', 30.0))  # seconds
...
def _check_event_names_dont_overlap():
    ...
    for name_a in event_names:
        assert name_a.endswith(
            'Event'), f'Event with name {name_a} does not end with "Event"'
