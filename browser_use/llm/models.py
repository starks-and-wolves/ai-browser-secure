...
def get_default_llm() -> 'BaseChatModel':
	"""
	Get the default LLM instance from DEFAULT_LLM_MODEL environment variable.

	Returns:
	    LLM instance configured with the default model from .env

	Raises:
	    ValueError: If DEFAULT_LLM_MODEL is not set or model is not recognized

	Example:
	    # Set DEFAULT_LLM_MODEL=gpt-5-nano in .env
	    llm = get_default_llm()  # Returns ChatOpenAI(model='gpt-5-nano')
	"""
	...
	if not model_name:
		raise ValueError(
			'DEFAULT_LLM_MODEL not set in environment. '
			'Please set it in your .env file (e.g., DEFAULT_LLM_MODEL=gpt-5-nano)'
		)

	# If model_name contains a dash (like gpt-5-nano), it's a direct model name
	# Otherwise it might be our underscore format (like openai_gpt_5_nano)
	if '-' in model_name or '.' in model_name:
		# Direct model name format (e.g., "gpt-5-nano", "claude-3-5-sonnet-latest")
		...
		if model_name.startswith('gpt-') or model_name.startswith('o1-') or model_name.startswith('o3-') or model_name.startswith('o4-'):
			...
			return ChatMistral(model=model_name, api_key=api_key, base_url=base_url)
		elif model_name.startswith('llama-') or 'qwen' in model_name.lower():
			api_key = os.getenv('CEREBRAS_API_KEY')
			return ChatCerebras(model=model_name, api_key=api_key)
		elif model_name.startswith('bu-'):
			api_key = os.getenv('BROWSER_USE_API_KEY')
			return ChatBrowserUse(model=model_name, api_key=api_key)
		else:
			# Default to OpenAI for unknown formats
			api_key = os.getenv('OPENAI_API_KEY')
			return ChatOpenAI(model=model_name, api_key=api_key)
	else:
		# Underscore format (e.g., "openai_gpt_5_nano") - use existing function
