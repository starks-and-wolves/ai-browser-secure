...
class TokenCost:
	...
	def register_llm(self, llm: BaseChatModel) -> BaseChatModel:
		...
		if instance_id in self.registered_llms:
			...
			logger.debug(f'LLM instance {instance_id} ({provider}_{model}) is already registered')
			return llm
		...
		async def tracked_ainvoke(messages, output_format=None, **kwargs):
			...
			if result.usage:
				...
				usage = token_cost_service.add_usage(model_name, result.usage)

				logger.debug(f'Token cost service: {usage}')

				create_task_with_error_handling(
					token_cost_service._log_usage(model_name, usage), name='log_token_usage', suppress_exceptions=True
				)
		...
		try:
			...
			logger.warning(f'Failed to register LLM instance {instance_id} for token tracking: {e}')
			# Remove from registered set since tracking failed
			del self.registered_llms[instance_id]
