...
if TYPE_CHECKING:
	from browser_use.browser.watchdogs.user_approval_watchdog import PermissionDeniedError
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		model_name = self.current_llm_model.lower()
		...
		logger.debug(
			...
			f' extraction_model={getattr(self.settings.page_extraction_llm, "model", getattr(self.settings.page_extraction_llm, "model_name", "Unknown")) if self.settings.page_extraction_llm else "Unknown"}'
		...
		is_browser_use_model = 'browser-use/' in self.current_llm_model.lower()
...
# 		In AWI mode with active manager, skip expensive browser state collection
		...
		# Re-inject AWI context on every step (context messages are cleared each step)
		if await self._is_awi_available():
			awi_context = self._get_awi_context_message()
			from browser_use.llm.messages import SystemMessage
			awi_system_message = SystemMessage(content=awi_context)
			self._message_manager._add_context_message(awi_system_message)
	...
	async def _get_next_action(self, browser_state_summary: BrowserStateSummary) -> None:
		...
		self.logger.debug(
			f'🤖 Step {self.state.n_steps}: Calling LLM with {len(input_messages)} messages (model: {self.current_llm_model})...'
		)
	...
	async def _execute_actions(self) -> None:
		...
		# Update user approval watchdog context before executing actions
		# This provides the LLM's plan, reasoning, and step info to the approval popup
		await self._update_approval_watchdog_context()

		result = await self.multi_act(self.state.last_model_output.action)
		self.state.last_result = result

	async def _update_approval_watchdog_context(self) -> None:
		"""Update the user approval watchdog with current agent context.

		This provides the watchdog with:
		- Current task description
		- LLM's current goal/next_goal
		- LLM's reasoning/thinking
		- Memory state
		- Step number and max steps
		- List of planned actions for this step
		"""
		if self.browser_session is None:
			return

		watchdog = self.browser_session._user_approval_watchdog
		if watchdog is None:
			return
		...
		# Extract action descriptions for display
		action_descriptions = []
		for action in model_output.action:
			# Get the action name and key parameters
			action_dump = action.model_dump(exclude_unset=True)
			for action_name, params in action_dump.items():
				if params is not None:
					if isinstance(params, dict):
						# Format: "action_name: key=value, ..."
						param_str = ', '.join(f'{k}={v}' for k, v in list(params.items())[:3])
						action_descriptions.append(f'{action_name}: {param_str}')
					else:
						action_descriptions.append(f'{action_name}: {params}')

		# Build reasoning from thinking and evaluation
		reasoning_parts = []
		if model_output.thinking:
			reasoning_parts.append(model_output.thinking)
		if model_output.evaluation_previous_goal:
			reasoning_parts.append(f'Previous: {model_output.evaluation_previous_goal}')
		reasoning = ' | '.join(reasoning_parts) if reasoning_parts else None

		# Update the watchdog context
		# Note: max_steps is not available here as it's passed to run(), so we omit it
		watchdog.update_agent_context(
			task=self.task,
			current_goal=model_output.next_goal,
			reasoning=reasoning,
	...
	async def _handle_step_error(self, error: Exception) -> None:
		...
		# Handle PermissionDeniedError specially - smart denial handling
		from browser_use.browser.watchdogs.user_approval_watchdog import PermissionDeniedError

		if isinstance(error, PermissionDeniedError):
			await self._handle_permission_denied(error)
			return
		...
		if 'Could not parse response' in error_msg or 'tool_use_failed' in error_msg:
			...
			self.logger.log(log_level, f'Model: {self.current_llm_model} failed')
	...
	async def _handle_permission_denied(self, error: 'PermissionDeniedError') -> None:
		"""Handle permission denied errors with smart re-planning.

		This method implements:
		1. If critical step denied and no re-plan attempted: inject re-plan context for LLM
		2. If critical step denied and already re-planned: exit the task
		3. If non-critical step denied: skip and continue with alternative approach
		"""

		self.logger.warning(f'⛔ Permission denied: {error.action_type}')

		# Check if we should exit (critical step denied after re-plan attempt)
		if error.should_exit():
			self.logger.error(
				f'🛑 Critical permission denied after re-plan attempt. '
				f'Cannot complete task without access to: {error.action_type}'
			)
			# Set a result that indicates task cannot be completed
			self.state.last_result = [
				ActionResult(
					error=f'Task cannot be completed: Permission denied for critical action "{error.action_type}". '
					f'The user denied this permission twice. Please try a different approach or task.',
					include_in_memory=True,
				)
			]
			# Force task to end by setting done flag
			if self.state.last_model_output:
				self.state.last_model_output.action = []  # Clear remaining actions
			return

		# Check if we should re-plan (critical step, first denial)
		if error.should_replan():
			self.logger.info('🔄 Critical permission denied. Requesting LLM to find alternative approach...')
			# Increment re-plan count in watchdog
			if self.browser_session and self.browser_session._user_approval_watchdog:
				goal_key = error.current_goal or error.url or error.action_type
				self.browser_session._user_approval_watchdog.increment_replan_count(goal_key)

			# Provide context for re-planning in the result
			replan_context = error.get_replan_context()
			self.state.last_result = [
				ActionResult(
					error=f'Permission denied - please find an alternative approach.\n{replan_context}',
					include_in_memory=True,
				)
			]
			# Don't increment consecutive_failures for re-plan attempts
			return

		# Non-critical step denied - just skip and continue
		self.logger.info(f'⏭️ Non-critical permission denied for {error.action_type}. Skipping step.')
		self.state.last_result = [
			ActionResult(
				error=f'Permission denied for {error.action_type}. Skipping this step and continuing.',
				include_in_memory=True,
			)
		]
		# Don't increment consecutive_failures for skipped steps
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			...
			self.logger.info(
				f'Starting a browser-use agent with version {self.version}, with provider={provider} and model={self.current_llm_model}'
			)
	...
	def _log_agent_event(self, max_steps: int, agent_run_error: str | None = None) -> None:
		...
		token_summary = self.token_cost_service.get_usage_tokens_for_model(self.current_llm_model)
	...
	def _get_awi_context_message(self) -> str:
		"""
		Generate context message for AWI mode.

		Returns:
			System message explaining AWI capabilities
		"""
		if not self.awi_manager or not self._awi_manifest:
			return ""

		awi_info = self._awi_manifest.get('awi', {})
		capabilities = self._awi_manifest.get('capabilities', {})
		...
		message = f"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                         🚀 AWI MODE IS NOW ACTIVE 🚀                         ║
╚══════════════════════════════════════════════════════════════════════════════╝

CRITICAL: You are connected to {awi_info.get('name', 'AWI')} via Agent Web Interface (AWI).

⚠️  IMPORTANT INSTRUCTIONS - READ CAREFULLY:

1. DO NOT use browser actions (navigate, click, type, scroll, etc.)
2. DO NOT try to interact with HTML elements or the DOM
3. DO NOT attempt to parse web pages or screenshots
4. The browser state you see is IRRELEVANT - ignore it completely

Instead, use the 'awi_execute' tool to make API calls:

📍 AWI Configuration:
  • Base URL: {base_url}
  • Available Operations: {', '.join(allowed_ops)}

🔧 How to Use awi_execute:

Use the awi_execute action with these parameters (YOU decide all values):
  • operation: Choose from {{{', '.join(allowed_ops)}}} based on what you need
  • endpoint: The API path (e.g., /posts, /posts/{{id}}/comments, /search)
  • method: HTTP method (GET, POST, PUT, PATCH, DELETE)
  • params: Query parameters as dict (e.g., {{"sort": "popular", "limit": 3}})
  • body: Request body as dict for POST/PUT (e.g., {{"content": "text", "title": "..."}})

⚠️  CRITICAL: When using POST/PUT/PATCH methods, you MUST provide a body dict with required fields!
    DO NOT send body={{}} - that will cause validation errors.
    Always include the actual data (e.g., body={{"content": "Great post!"}})
...
Example 2 - Create/Write operation (e.g., adding a comment):
  awi_execute(
    operation="create",
    endpoint="/posts/507f1f77bcf86cd799439011/comments",
    method="POST",
    body={{"content": "Great post!", "authorName": "Agent"}}
  )

  IMPORTANT: For POST/PUT requests, you MUST include a 'body' parameter with the required fields.
  - For comments: body must include "content" (required) and optionally "authorName"
  - For posts: body must include "title" and "content" (both required)
  - Empty body {{}} will cause validation errors!
...
⚠️  COMMON MISTAKE - Learn from this!

Task: "Add a comment 'Great post!' to post 123"
...
❌ WRONG (will fail with validation error):
  awi_execute(
    ...
    method="POST",
    body={{}}  ← EMPTY! This causes: "content: Comment content is required"
  )
...
"""
	...
	async def _is_awi_available(self) -> bool:
		...
		return self.awi_mode and self.awi_manager is not None

	async def get_awi_session_state(self) -> dict[str, Any] | None:
		"""Get current AWI session state (if AWI mode is active)."""
		if await self._is_awi_available():
			try:
				return await self.awi_manager.get_session_state()  # type: ignore
			except Exception as e:
				self.logger.warning(f'Failed to get AWI session state: {e}')
		return None

	async def get_awi_action_history(self, limit: int = 20) -> dict[str, Any] | None:
		"""Get AWI action history/trajectory (if AWI mode is active)."""
		if await self._is_awi_available():
			try:
				return await self.awi_manager.get_action_history(limit=limit)  # type: ignore
			except Exception as e:
	...
	async def _try_awi_discovery(self, url: str) -> bool:
		...
		This runs BEFORE traditional browser navigation to check if AWI is available.
		Flow:
		1. Discover AWI
		2. Check agent registry for existing credentials
		3. If credentials exist: reuse them (skip registration)
		4. If not: show permission dialog and register new agent

		Args:
			url: The URL to check for AWI support

		Returns:
			True if AWI was discovered and activated, False otherwise
		"""
		if not self.awi_mode:
			return False
		...
		try:
			...
			async with AWIDiscovery() as discovery:
				...
				self.logger.info(f"✅ AWI discovered: {manifest.get('awi', {}).get('name')}")
				self._awi_manifest = manifest

			# Step 2: Check agent registry for existing credentials
			from browser_use.agent_registry import agent_registry

			existing_cred = agent_registry.get_credentials(url)

			if existing_cred:
				# Reuse existing agent credentials
				self.logger.info(f'🔄 Found existing agent credentials: {existing_cred.agent_name}')
				self.logger.info(f'   Agent ID: {existing_cred.agent_id}')
				self.logger.info(f'   Last used: {existing_cred.last_used or "never"}')
				...
				self.awi_manager.agent_info = {
					'id': existing_cred.agent_id,
					'name': existing_cred.agent_name,
					'permissions': existing_cred.permissions,
					'agentType': existing_cred.agent_type,
					'framework': existing_cred.framework,
				}
				self.awi_manager.api_key = existing_cred.api_key

				# Update usage tracking
				agent_registry.update_last_used(existing_cred.agent_id)

				AWIPermissionDialog.show_awi_mode_banner()
				self.logger.info(f'✅ AWI Mode activated with existing agent')
				self.logger.info(f"   Permissions: {', '.join(existing_cred.permissions)}")
			...
			# Step 5: Store credentials in registry for future reuse
			agent_registry.store_credentials(
				agent_id=agent_info['id'],
				agent_name=agent_info['name'],
				domain=url,
				api_key=agent_info['apiKey'],
				permissions=agent_info['permissions'],
				awi_name=manifest.get('awi', {}).get('name', ''),
				description=f"Browser-use agent: {self.task[:100]}",
				agent_type='browser-use',
				framework='python',
			...
			self.logger.info(f"   Permissions: {', '.join(approval['permissions'])}")
			self.logger.info(f'💾 Credentials stored in registry for future reuse')

			return True

		except Exception as e:
			self.logger.error(f'❌ AWI registration failed: {e}')
			self.logger.info('Falling back to traditional DOM parsing')
			self.awi_manager = None
			self._awi_manifest = None
			return False

	def _register_generic_awi_tool(self) -> None:
		"""
		Register the generic AWI execution tool.

		This is a single, task-agnostic tool that allows the LLM to execute
		any API call based on the task and the AWI manifest's available operations.

		The LLM decides:
		- Which operation to use (from manifest's allowed_operations)
		- Which endpoint to call (based on API structure)
		- Which HTTP method to use (GET, POST, PUT, DELETE, etc.)
		- What parameters/body to send (based on task requirements)
		"""
		if not self.awi_manager:
			return

		from browser_use.awi import AWIExecuteAction, awi_execute

		# Register the single generic AWI tool
		# The LLM will use this to execute any API call it plans
		@self.tools.registry.action(
			'Execute a generic AWI API call. Use this to interact with the AWI-enabled website via structured API calls instead of browser actions. The LLM decides all parameters based on the task and available operations from the manifest.',
			param_model=AWIExecuteAction,
		)
		async def awi_execute_bound(params: AWIExecuteAction):
			"""Generic AWI API executor - LLM decides all parameters."""
			return await awi_execute(params, self.awi_manager)
	...
	async def run(
		...
		try:
			...
			# Skip initial navigation if AWI is active (agent will use API calls instead)
			if not awi_is_active:
				...
				self.logger.info('⏭️  Skipping initial browser navigation (AWI mode active)')
