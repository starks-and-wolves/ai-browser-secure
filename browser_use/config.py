...
class FlatEnvConfig(BaseSettings):
	...
	DEFAULT_LLM_MODEL: str = Field(default='gpt-5-nano')
...
class AgentCredentialEntry(DBStyleEntry):
	"""
	AWI Agent credential entry for persistent authentication.

	Stores credentials for registered agents across runs, allowing:
	- Agent reuse without re-registration
	- Session persistence
	- Credential rotation
	...
	agent_id: str = Field(description='Unique agent ID from AWI registration')
	agent_name: str = Field(description='Human-readable agent name')

	# Domain/service identification
	domain: str = Field(description='Domain or URL where agent is registered (e.g., localhost:5000)')
	awi_name: str = Field(default='', description='Name of the AWI service from manifest')

	# Authentication credentials
	api_key: str = Field(description='API key for authentication')

	# Permissions and capabilities
	permissions: list[str] = Field(default_factory=list, description='Granted permissions (e.g., read, write)')

	# Optional metadata
	description: str | None = Field(default=None, description='Agent description')
	agent_type: str = Field(default='browser-use', description='Type of agent')
	framework: str = Field(default='python', description='Framework used')

	# Session tracking
	last_used: str | None = Field(default=None, description='Last usage timestamp (ISO format)')
	session_count: int = Field(default=0, description='Number of sessions using this agent')

	# Credential lifecycle
	expires_at: str | None = Field(default=None, description='Expiration timestamp (ISO format)')
	is_active: bool = Field(default=True, description='Whether credentials are still valid')

	# Additional metadata
	manifest_version: str | None = Field(default=None, description='AWI manifest version used')
	notes: str | None = Field(default=None, description='User notes about this agent')
...
class DBStyleConfigJSON(BaseModel):
	...
	agent_credentials: dict[str, AgentCredentialEntry] = Field(
		default_factory=dict,
		description='AWI agent credentials for persistent authentication'
	)
